...
def barplot_dotline(df, save_dir, save_title,  title, xlabel, xticks, ylabel, yname, ymin=None, ymax=None, yinterval=None, yticks=None,
                    barhuename=None, barhuenameval=None, barhue=None, barwidth = 0.5, ci=None, errcolor='black', pvals = None,
                       ...
                       linehue=None, linehuewidth=2, linehuecolor='black', linehuemean = None, linehuemeanwidth=3, linehuemeancolor='gray',
                       linex=None, linexwidth=2, linexcolor="deep", dpi=80, width=1.5, height=8, titlesize=20, labelsize=20, ticksize=18, 
   ...
   linex: draw lines betwen bars of different xticks, if linex=True 
   """
   ...
   if plot_type == 'bar':
       ...
       ax=sns.violinplot(x='X', y='Y', data=df, ci=ci, hue=barhuename, 
                         split=split, cut = cut, scale=scale, inner=inner,
                        orient="v", palette=barhue, saturation=0.5)
   elif plot_type == 'box+strip':
       
       ax=sns.stripplot(x='X', y='Y', data=df, jitter=False, dodge=True, hue=barhuename, palette=dotcolor,
                        linewidth=dotedgewidth,size=dotsize, alpha=0.15, zorder=2)#edgecolor="black", 
       ax= sns.boxplot(x='X', y='Y', data=df, hue=barhuename, palette=barhue,zorder=1, fliersize=0,
   ...
   if not linex is None:
       #plt.legend([], [], frameon=False)
       ax = sns.lineplot(x='X', y='Y', data=df,err_style=None,
                      hue = dothuename, markers=True, marker='o',markersize=8, palette = linexcolor) 
       plt.legend(bbox_to_anchor=(1,1), fontsize=ticksize, frameon=False)
   # draw dots for different subjects/trial
   elif not dot is None: 
       
       # only dots
       if linehue is None:
           if barhuename is None:
               ax = sns.stripplot(x='X', y='Y', data=df, dodge=False, orient="v",
                          hue = dothuename,size=dotsize, palette = dotcolor, color=dotcolor2) 
           else:
               ax = sns.stripplot(x='X', y='Y', data=df, dodge=True, orient="v",
                          hue = barhuename, size=dotsize, palette = dotcolor, color=dotcolor2) 
       ...
       # connect lines between dots
           ...
           for i in range(len(ax_list)):
               ...
               if i>=0 and i<interval:
                    ...
                    # draw lines connecting bars' top       
                    if not linehuemean is None:
                        h1 = p1.get_height()
                        h2 = p2.get_height()    
                        plt.plot(x1,h1,dotstyle,markeredgecolor=linehuemeancolor,markerfacecolor=[1,1,1], markersize=dotsize+2,markeredgewidth=dotsize/2+1)
                        plt.plot(x2,h2,dotstyle,markeredgecolor=linehuemeancolor,markerfacecolor=[1,1,1], markersize=dotsize+2,markeredgewidth=dotsize/2+1)
                        plt.plot([x1,x2],[h1,h2], color=linehuemeancolor, linewidth=linehuemeanwidth)
