...
from eval.identify_line_orientation import principalOrient_Radon_var
...
save_title = 'Principal_orientation_global'
...
for i, image in enumerate(images):
    ...
    [orientation, w] = principalOrient_Radon_var(img, r_list, theta)
    ...
    Inducer_orientation.append(orient_inducer_list[i])
    Illusory_orientation.append(orient_illus_list[i])
    ...
    similarity_inducer = abs(np.cos((orient_inducer_list[i]-orientation)/180*np.pi))
    similarity_illusory = abs(np.cos((orient_illus_list[i]-orientation)/180*np.pi))
...
PrOr = pd.DataFrame.from_dict({'Principal orientation': Principal_orientation, 
                              ...
                              'reconType':reconType, 'Line space':Line_space, 
